...
for input_file in input_files:
        ...
        for layer2 in range(1, layer1):
           for learning_rate in learning_rates:
               for learning_rate_init in learning_rate_inits:
                    model_name = '{}_{}_{}_{}_{}'.format(layer1,layer2,'sgd', learning_rate, 'relu')
                    layer1_list.append(layer1)
                    layer2_list.append(layer2)
                    solver_list.append('sgd')
                    ...
                    model = MLPRegressor(
                                hidden_layer_sizes=(layer1,layer2), activation='relu', solver='sgd',
                                learning_rate=learning_rate, learning_rate_init = learning_rate_init, max_iter=500,
                                random_state=9, early_stopping=True)
                    start = time.time()
                    model.fit(X_train, y_train)
                    time_to_fit.append(time.time() - start)
                    with open('D:/sgdTest2/models/' + model_name +"_"+ input_file, 'wb') as f:
                         pickle.dump(model, f)
                    ...
                    with open('D:/sgdTest2/predicts_test/' + model_name +"_"+ input_file, 'wb') as f:
                    ...
                    with open('D:/sgdTest2/predicts_train/' + model_name + "_"+ input_file, 'wb') as f:
                         pickle.dump(concat_train, f)
                    # with open("time_and_loss_train.txt", 'a') as file:
                    #     file.write(model_name +" "+ input_file + loss_train + '\n')
        print (layer1)

    d = { 'solver': solver_list, 'layer1': layer1_list , 'layer2': layer2_list, 'activation': activation_list,'max_iter':max_iter_list,'learning_rate': learning_rate,'learning_rate_init': learning_rate_init_list,'time_fit': time_to_fit,
    ...
    dump("D:/sgdTest2/time_and_loss_" + input_file, df)
